...
RectProperties CalculateRectProperties (Rect *givenRect)
{
    bool is_rect = false;
    float diagonal = sqrtf(
        ((givenRect->a.x - givenRect->b.x) * (givenRect->a.x - givenRect->b.x)) +
        ((givenRect->a.y - givenRect->b.y) * (givenRect->a.y -givenRect->b.y))
    );
